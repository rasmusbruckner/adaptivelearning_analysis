""" Figure S9: Model comparison experiment 1

1. Load data and compute cumulated BICs
2. Prepare figure
3. Plot BIC and pEP
...
# model_1 = pd.read_pickle('al_data_prev/model_exp_1_log_25sp.pkl')
model_1 = pd.read_pickle('al_data/estimates_first_exp_10_sp.pkl')
model_2 = pd.read_pickle('al_data/estimates_first_exp_no_pers_10_sp.pkl')

# Compute for how many participants perseveration model fit the data better
model_diff = model_1['BIC'] - model_2['BIC']
print(sum(model_diff > 0))  # in 121 out of 129, model with perseveration fit the data better.
...
ax_0.yaxis.set_label_coords(ylabel_dist, 0.5)

# Add y-axis text
f.text(0.15, 0.93, r'$\times 10^4$', size=8, rotation=0, color='k', ha="center", va="center")
...
plt.subplots_adjust(left=0.2, bottom=0.15, right=None, top=None, wspace=0.5, hspace=0.5)
sns.despine()

texts = ['a', 'b']  # label letters
label_subplots(f, texts, x_offset=0.2, y_offset=0.0)
