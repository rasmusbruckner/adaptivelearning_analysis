""" Figure S11: Parameter recovery first experiment
...
param_recov = pd.read_pickle('al_data/param_recov_exp1.pkl')
true_params = pd.read_pickle('al_data/true_params_recov_exp1.pkl')
...
plt.xlim(-35, 35)
plt.ylim(-35, 35)
plt.xticks(np.arange(-35, 35, 15))
plt.yticks(np.arange(-35, 35, 15))
...
plt.xlim(-2, 13)
plt.ylim(-2, 13)
plt.xticks(np.arange(-2, 14, 3))
plt.yticks(np.arange(-2, 14, 3))
...
# q
plt.subplot(338)
plt.plot(true_params['q'].values, param_recov['q'].values, '.', color='k')
plt.title('Reward Bias')
plt.xlim(-0.55, 0.55)
plt.ylim(-0.55, 0.55)
