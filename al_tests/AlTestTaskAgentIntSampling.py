""" Task-Agent-Interaction Sampling Model: Unit and integration tests """
...
class TestTaskAgentIntSampling(unittest.TestCase):
    """ This class definition implements the unit tests for the task-agent-interaction of the sampling model
    ...
    def test_integration(self):
        ...
        agent_vars = AgentVarsSampling()
        agent_vars.criterion = 0.2
        agent_vars.n_samples = 5
        agent = AlAgentSampling(agent_vars)

        # Run task-agent interaction
        agent.reinitialize_agent(seed=1)
        df_data = task_agent_int_sampling(df_subj, agent, agent_vars, seed=1)

        # savename = 'al_tests/test_task_agent_int_sampling.pkl'
        # df_data.to_pickle(savename)

        # Load test data
        test_data = pd.read_pickle('al_tests/test_task_agent_int_sampling.pkl')
